...
const validerArticle = (event, data, idx) => {
  ...
  axios.patch('https://corona-watch-esi.herokuapp.com/content/articles/'+data.id, data1)
  .then((response) => {
    console.log(response);
    var validerBtn=document.getElementsByClassName('validerBtnArticle');
    validerBtn[idx].style.display='none';
    var validerBtn=document.getElementsByClassName('validerBtnArticleDisabled');
...
const supprimerPost = (event, data, idx) => {
  const data1 ={
   ...
   .then((response) => {
     console.log(response);
     var supprimerBtn=document.getElementsByClassName('supprimerBtnArticle');
     supprimerBtn[idx].style.display='none';
     var supprimerBtn=document.getElementsByClassName('supprimerBtnArticleDisabled');
...
const headCells = [
  ...
  { id: 'sup', numeric: true, disablePadding: false, label: "" },
  { id: 'val', numeric: true, disablePadding: false, label: '' },
];
                ...
                >
                    ...
                    {stableSort(data, getComparator(order, orderBy))
                        ...
                        return (
                            ...
                            <TableCell align="left" style={{width:'20%',paddingLeft:'20px'}}>{row.writer}</TableCell>

                            <TableCell align="left" >
                            {!row.verified ? 
                              <Button className={'validerBtnArticle'} variant="contained" color="primary" style={{backgroundColor:'#4E73DF',borderRadius:'25px'}} onClick={event => validerArticle(event, row, idx)}>
                                  Valider
                              </Button>: <Button className={'validerBtnArticle'} style={{borderRadius:'25px'}} variant="contained" disabled>valider</Button>}
                              <Button className={'validerBtnArticleDisabled'} variant="contained" style={{ display:'none',borderRadius:'25px'}} disabled>verifier</Button>
                            </TableCell>

                            <TableCell align="left" >
                            {!row.deleted ? 
                              <Button className={'supprimerBtnArticle'} variant="contained" color="secondary" style={{borderRadius:'25px'}} onClick={event => supprimerPost(event, row, idx)}>
                                  Supprimer
                              </Button>: <Button className={'supprimerBtnArticle'} style={{borderRadius:'25px'}} variant="contained" disabled>Supprimer</Button>}
                              <Button className={'supprimerBtnArticleDisabled'} variant="contained" style={{ display:'none', borderRadius:'25px'}} disabled >Supprimer</Button>
                            </TableCell>                            
